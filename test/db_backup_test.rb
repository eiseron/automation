...
module EiseronAutomation
  class DbBackupTest < Minitest::Test
    ...
    def test_dumps_the_product_database_over_the_network_with_the_password
      ...
      assert_includes dump, "app_prod"
      assert_includes dump, "app-db"
    ...
    def test_uploads_a_timestamped_object_under_the_product_prefix
      ...
      assert_equal "app/2026-06-13T030000Z.sql.age", store.uploaded.fetch(0)[:key]
    end
    ...
    def test_prunes_remote_objects_older_than_the_retention_window
      old = "app/2026-05-01T030000Z.sql.age"
      recent = "app/2026-06-12T030000Z.sql.age"
    ...
    def test_honours_a_custom_retention_window
      old = "app/2026-06-10T030000Z.sql.age"
    ...
    def test_appends_new_key_to_history
      store = FakeStore.new(history_text: "app/2026-06-12T030000Z.sql.age\n")
      backup(store, env).run
      history_write = store.texts_written.find { |w| w[:key] == "app/history" }
      assert history_write
      keys = history_write[:text].lines.map { |line| line.strip.split("\t").first }
      assert_includes keys, "app/2026-06-12T030000Z.sql.age"
      assert_includes keys, "app/2026-06-13T030000Z.sql.age"
    end
    ...
    def test_creates_history_when_none_exists
      ...
      assert_equal "app/2026-06-13T030000Z.sql.age\t#{sha256}\n", history_write[:text]
    end
    ...
    def test_records_the_sha256_of_the_uploaded_object_in_history
      ...
      assert_equal "app/2026-06-13T030000Z.sql.age", key
